...
import {
    ...
    fromWString } from './wstring.js';
import getWindowsSystemErrorText from './windows-system-error.js';
...
class RegistryKey {
    ...
    create() {
        ...
        const { rootKey, subKey } = this.parsePath();
        if (rootKey !== null) {
            //verbose(`Creating key: ${subKey} under ${rootKey}...`);
    ...
    getValue(name) {
        ...
        if (this.keyData.handle !== null) {
            ...
            if (result === 0) {
                ...
                if (result === 0) {
                    //verbose(`ValueType of ${name}: ${valueType.deref()}`);
                    //verbose(`Length of ${name}: ${buffer.length}`);
    ...
    closeKeyInternal(keyData) {
        if (keyData.handle !== null) {
            ...
            if (result !== 0) {
                ...
                keyData.handle = null;
                //verbose(`Closed key "${keyData.path}}"`);
            }
        } else {
    ...
    stopMonitorInternal(tokenData) {
        if (tokenData.callback !== null) {
            ...
            if (monitoredKey) {
                ...
                //verbose(`Stopped monitoring key "${tokenData.path}}"`);
            } else {
    ...
    stopMonitoredKeyInternal(monitorData) {
        if (monitorData.waitHandle !== null) {
            ...
            if (result === 0) {
                ...
                this.updateMonitoredKeysArray();
                //verbose(`Monitored key "${keyData.path}}" stopped`);
            }
        } else {
    ...
    monitorCheck() {
        while (true) {
            ...
            if (result === WaitForMultipleObjectsResult.WAIT_FAILED) {
                ...
                //verbose('monitorCheck: WaitForMultipleObjects timed out. No key has signaled.');
                break;
